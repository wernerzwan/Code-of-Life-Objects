...
int maitresse_threshold = 180;
long rake_threshold_interval = 100;
int rake_threshold = 160;
long chair_threshold_interval = 100;
int chair_threshold = 200;
long broom_threshold_interval = 100;
...
void loop()
{
  ...
  if (currentMillis - previousMillis > interval)
  {
    ...
    loopnum = map(o6, 0, 255, 200, 1200);
    for (i = 0; i < loopnum; i++)
    {

      int broom = broom_func(o3, broom_threshold);
      int rake = rake_func(o3, rake_threshold);
      int mattress = mattress_func(o5, maitresse_threshold);
...
//      Serial.print(",");
    ...
    if (currentMillis - previousIronBoardThresholdIntervalMillis > ironboard_threshold_interval)
    {
      ...
      iron_board_threshold = random(200, 240);
    }
    if (currentMillis - previousMattressThresholdIntervalMillis > maitresse_threshold_interval)
    {
      ...
      maitresse_threshold = random(190, 240);
    }
    ...
    if (currentMillis - previousChairThresholdIntervalMillis > chair_threshold_interval)
    {
      ...
      chair_threshold = random(190, 240);
    }
    if (currentMillis - previousBroomThresholdIntervalMillis > broom_threshold_interval)
    {
      ...
      broom_threshold = random(150, 240);
    }
    if (i2 > 20000) {
      Serial.println("Resetting now");
      delay(200);
...
int mattress_func(int raw_value, int threshold)
{
  ...
  {
    NewValue = map(raw_value, 0, 255, 0, 255);
  }
  return NewValue;
}
...
int rake_func(int raw_value, int threshold)
{
  ...
  {
    NewValue = map(raw_value, 0, 255, 160, 255);
  }
  return NewValue;
}
...
int chair_func(int raw_value, int threshold)
{
  ...
  {
    NewValue = map(raw_value, 0, 255, 210, 255);
  }
  return NewValue;
}
...
int broom_func(int raw_value, int threshold)
{
  ...
  {
    NewValue = map(raw_value, 150, 255, 100, 255);
  }
  return NewValue;
}
