...
long ironboard_threshold_interval = 100;
int iron_board_threshold = 80;
long maitresse_threshold_interval = 100;
int maitresse_threshold = 70;
...
void loop()
{
  ...
  if (currentMillis - previousMillis > interval)
  {
    ...
    for(i = 0; i < 5000; i ++) 
    {
        ...
        loopnum = map(o6, 0, 255, 200, 800);
        for (i = 0; i < loopnum; i++)
        {
          
    int broom = broom_func(o3, broom_threshold);
    int rake = rake_func(o5, rake_threshold);
...
//    }
  ...
  if (currentMillis - previousIntervalMillis > interval_interval)
  {
    ...
    interval = map(o5, 0, 255, 300, 1000);
  }
  if (currentMillis - previousIronBoardThresholdIntervalMillis > ironboard_threshold_interval)
  {
    ...
    iron_board_threshold = random(1, 70);
  }
  if (currentMillis - previousMattressThresholdIntervalMillis > maitresse_threshold_interval)
  {
    ...
    maitresse_threshold = random(1, 100);
  }
  ...
  if (currentMillis - previousBroomThresholdIntervalMillis > broom_threshold_interval)
  {
    ...
    broom_threshold = random(100, 240);
  }
    }
...
int ironboard(int raw_value, int threshold)
{
  ...
  {
    NewValue = raw_value;
    NewValue = map(raw_value, 0, 255, 250, 255);
  }
  return NewValue;
}
