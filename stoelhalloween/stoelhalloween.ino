...
void loop()
{
  ...
  if (currentMillis - previousMillis > interval)
  {
    ...
    for (i = 0; i < loopnum; i++)
    {
      int mattress = mattress_func(o3, maitresse_threshold);
...
//      Serial.print(",");
    ...
    if (currentMillis - previousIntervalMillis > interval_interval)
    {
      ...
      interval = map(o5, 0, 255, 200, 1500);
    }
    if (currentMillis - previousIronBoardThresholdIntervalMillis > ironboard_threshold_interval)
    {
      ...
      iron_board_threshold = random(170, 240);
    }
...
int mattress_func(int raw_value, int threshold)
{
  ...
  {
    NewValue = map(raw_value, 0, 255, 120, 255);
  }
  return NewValue;
}
